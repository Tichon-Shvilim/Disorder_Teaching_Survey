...
import { FormAPIServiceV2 } from './Api-Requests/FormAPIServiceV2';
import type { QuestionnaireTemplateV2, FormAnswerV2, FormNodeV2, OptionV2, VisibilityCondition } from './models/FormModelsV2';
import { toast } from 'react-toastify';
import type { RootState } from '../../store';

const FillFormV2: React.FC = () => {
  ...
  const [questionnaires, setQuestionnaires] = useState<QuestionnaireTemplateV2[]>([]);
  const [selectedQuestionnaireId, setSelectedQuestionnaireId] = useState<string>(questionnaireId || '');
  const [questionnaire, setQuestionnaire] = useState<QuestionnaireTemplateV2 | null>(null);
  const [answers, setAnswers] = useState<Record<string, FormAnswerV2>>({});
    ...
    const loadData = async () => {
      try {
        ...
        if (editSubmissionId) {
          ...
          const submission = await FormAPIServiceV2.getSubmission(editSubmissionId);
          const questionnaireData = await FormAPIServiceV2.getQuestionnaireTemplate(submission.questionnaireId);
          ...
          const formAnswers: Record<string, FormAnswerV2> = {};
          submission.answers.forEach((answer: FormAnswerV2) => {
          ...
          const data = await FormAPIServiceV2.getQuestionnaireTemplates();
  ...
  useEffect(() => {
    ...
    const fetchQuestionnaire = async () => {
      try {
        setLoading(true);
        const data = await FormAPIServiceV2.getQuestionnaireTemplate(selectedQuestionnaireId);
        
        setQuestionnaire(data);
        setShowQuestionnaireList(false);
        
        // Initialize answers for all questions in the structure
        const initialAnswers: Record<string, FormAnswerV2> = {};
        
        const extractQuestions = (nodes: FormNodeV2[], path: string[] = []) => {
          nodes.forEach(node => {
  ...
  const renderQuestion = (node: FormNodeV2): React.ReactNode => {
                ...
                >
                  <input
                    ...
                    onChange={() => {
                      ...
                      let newSelectedOptions: OptionV2[];
  ...
  const renderNodes = (nodes: FormNodeV2[], path: string[] = []): React.ReactNode => {
  ...
  const evaluateVisibility = (node: FormNodeV2, answers: Record<string, FormAnswerV2>): boolean => {
  ...
  const validateForm = (): boolean => {
    ...
    const validateNodes = (nodes: FormNodeV2[]) => {
      nodes.forEach(node => {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (currentSubmissionId) {
        ...
        await FormAPIServiceV2.updateSubmission(currentSubmissionId, {
        ...
        const result = await FormAPIServiceV2.submitForm(submission);
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      if (currentSubmissionId && !isDraft) {
        ...
        await FormAPIServiceV2.submitForm(submission);
