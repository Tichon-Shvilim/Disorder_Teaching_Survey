...
import type { FormSubmissionV2, QuestionnaireTemplateV2 } from './models/FormModelsV2';
import { toast } from 'react-toastify';
import { useSelector } from 'react-redux';
import type { RootState } from '../../store';

const ViewSubmissionsV2: React.FC = () => {
  ...
  const [selectedSubmission, setSelectedSubmission] = useState<FormSubmissionV2 | null>(null);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);
  const [submissionToDelete, setSubmissionToDelete] = useState<FormSubmissionV2 | null>(null);
    ...
    const fetchData = async () => {
      try {
        ...
        const [submissionsResult, questionnairesData] = await Promise.all([
          FormAPIServiceV2.getSubmissions({ studentId }),
          FormAPIServiceV2.getQuestionnaireTemplates()
        ]);
  ...
  const handleFillNewForm = (questionnaireId: string) => {
    navigate(`/layout/forms/v2/fill/${questionnaireId}`, {
      state: { 
        studentId: studentId, 
        studentName
      }
  ...
  const handleViewSubmission = (submission: FormSubmissionV2) => {
    setSelectedSubmission(submission);
  };

  const handleEditSubmission = (submission: FormSubmissionV2) => {
    if (submission.status === 'completed') {
      toast.warning('Completed submissions cannot be edited. You can only edit draft submissions.');
      return;
    }

    navigate(`/layout/forms/v2/fill/${submission.questionnaireId}`, {
      state: { 
        studentId: studentId, 
        studentName,
  ...
  const handleDeleteSubmission = (submission: FormSubmissionV2) => {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await FormAPIServiceV2.deleteSubmission(submissionToDelete._id);
...
export default ViewSubmissionsV2;
