/**
 * Script to create Hebrew questionnaire in the enhanced questionnaire system
 * Converts hebrew_questionnaire_data.json to FormNode structure and creates via API
 */

import type { FormNode, CreateQuestionnaireRequest, GraphSettings } from './my-app/src/components/formManagement/models/FormModels';

// Hebrew questionnaire data converted to FormNode structure
const hebrewQuestionnaireStructure: FormNode[] = [
  {
...
// Graph settings for the questionnaire
const hebrewGraphSettings: GraphSettings = {
  colorRanges: [
...
export const hebrewQuestionnaireRequest: CreateQuestionnaireRequest = {
  title: "שאלון הוראה לילדים עם הפרעות",
  description: "שאלון מקיף לבחינת שיטות הוראה והתנהגות כלפי ילדים עם הפרעות למידה והתנהגות",
  structure: hebrewQuestionnaireStructure,
  graphSettings: hebrewGraphSettings
};

console.log('Hebrew questionnaire structure created successfully!');
console.log('Total groups:', hebrewQuestionnaireStructure.length);
console.log('Total questions:', hebrewQuestionnaireStructure.reduce((count, group) => 
  count + group.children.filter(child => child.type === 'question').length, 0));
console.log('All questions are graphable:', hebrewQuestionnaireStructure.every(group => 
  group.children.every(child => child.type !== 'question' || child.graphable)));
