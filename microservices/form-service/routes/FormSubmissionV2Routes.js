...
const FormSubmissionV2 = require('../models/FormSubmissionV2');
...
async function validateQuestionIds(questionnaireId, answers) {
  try {
    const questionnaire = await QuestionnaireTemplateV2.findById(questionnaireId);
    if (!questionnaire) {
...
router.post('/v2/submit', authenticateJWT, authorizeRole(['Teacher', 'Therapist', 'Admin']), async (req, res) => {
  try {
    const { 
      studentId, 
      questionnaireId, 
    ...
    const formSubmission = new FormSubmissionV2({
      studentId,
...
router.get('/v2/submissions', authenticateJWT, async (req, res) => {
  try {
    ...
    // Fetch submissions
    const submissions = await FormSubmissionV2.find(query)
      .sort({ submittedAt: -1, createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .select('-answers'); // Exclude answers for list view performance

    const total = await FormSubmissionV2.countDocuments(query);
...
router.get('/v2/submissions/:id', authenticateJWT, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const submission = await FormSubmissionV2.findById(id);
...
router.put('/v2/submissions/:id', authenticateJWT, authorizeRole(['Teacher', 'Therapist', 'Admin']), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updatedSubmission = await FormSubmissionV2.findByIdAndUpdate(
      id,
      updateData,
...
router.delete('/v2/submissions/:id', authenticateJWT, authorizeRole(['Admin']), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const submission = await FormSubmissionV2.findByIdAndDelete(id);
